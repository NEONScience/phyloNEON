...
#' @param goldType defaults
#'
#' @return opens a web page in default browser
#'
#' @examples
#' \dontrun{
#' # To open a page on jgi GOLD online database :
#' openGOLD('BONA_013-O-20230710-COMP-DNA1')
#' }
#' @references
...
#   Hugh Cross (2025-10-10)
#     original creation
#'
#'
##############################################################################################

openGOLD <- function(sampleID, goldType = 'GOLD Sequencing Project ID', sourceTable = neon.metaDB){
  # add persistent link
  taxonPers <- "https://identifiers.org/gold:"
  # check gold type
  if(!goldType %in% c("GOLD Sequencing Project ID","GOLD Analysis Project ID","GOLD Study ID")){
    stop("You must select a GOLD ID,\n example: GOLD Sequencing Project ID")
  } else {
